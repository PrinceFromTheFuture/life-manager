...
/// The ink, as a printed mark — the same stamp language as the hub feed's
/// left bar, sized for a chip, a row, or a lookups list.
...
/// how it prints.
class StampInkPad extends StatelessWidget {
  const StampInkPad({
    super.key,
    required this.ink,
    ...
    final well = Material(
      ...
      child: InkWell(
        ...
        child: SizedBox(
          ...
          height: 44,
          child: selected
              ? AppIcon(SolarIcons.CheckCircle, size: 18, color: palette.paper)
              : null,
        ),
      ),
    );
...
Future<StampInk?> showStampInkPicker({
  required BuildContext context,
  StampInk? selected,
  String title = 'Ink',
}) {
  final palette = context.thermal;
  return showDialog<StampInk>(
    context: context,
